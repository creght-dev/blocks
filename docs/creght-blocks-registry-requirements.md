...
| 分发 / 部署 | **npm 发布**（见 §3.4；发布后通过稳定 HTTPS URL 提供 `registry.json` 与各条目 JSON） |
| 可见性 | **全部公开** |
...
> **已定稿**：`npm` 分发即以「发布 npm 包 + CDN（如 unpkg / jsDelivr）提供 HTTPS JSON」为准。**npm scope / 包名** 暂不命名，上线包前再定。
...
- **npm**：将包含 `registry.json`、构建产物与文档站所需资源 **发布为 npm 包**（包名待定）。消费者通过 **稳定 HTTPS URL** 安装，例如：  
...
- **文档站点**：公开展示板块列表、说明、预览或截图、安装命令与依赖说明。
- **可见性**：Registry 与文档均为 **公开**。

### 3.5 验收标准（可测）

1. **`registry.json` 与各条目 JSON** 可被公开 HTTPS 访问，且可被 `shadcn add` 使用。
2. **至少 5 个 Hero 条目** 在文档站列出并可按需安装。
3. **Creght（目标应用）**：在约定分支/环境中能通过文档说明 **成功引入并渲染** 安装的板块（无阻断性报错；样式与交互符合约定范围）。

---

## 4. 非目标（MVP）

- **私有 Registry / 登录鉴权**（当前不需要）。
- **可视化搭建器**（非本期范围）。
